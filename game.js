...
class Game {
    ...
    static getFilters() { // default implementation if not overridden
      return (() => {})
    }
    ...
    static playAction(state, action) {
      ...
      if (process.env.NODE_ENV != 'production') { // development or test
        if (JSON_delta.diff(this.replayAction(view, action, diffs), this.filter(newState)).length != 0) {
          throw new Error('Result of replaying the action did not match the new state')
        }
      }
